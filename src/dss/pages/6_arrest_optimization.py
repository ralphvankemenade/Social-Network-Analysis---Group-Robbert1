...
def page() -> None:
    ...
    if arrest_result is not None:
        ...
        # Display network coloured by department.  Build hover text to
        # include department assignment, community and centrality metrics.
        ...
        centralities_all = compute_centralities(G)
        comm_result = get_state("community_results").get(comm_method)
        hover_dict = {}
        for node in G.nodes():
            parts = [f"Department: {arrest_result.assignment[node]}"]
            if comm_result:
                parts.append(f"Community: {comm_result.labels.get(node, '')}")
            for col in centralities_all.columns:
                parts.append(f"{col}: {centralities_all.at[node, col]:.4f}")
            hover_dict[node] = "<br>".join(parts)
        display_network(
            G,
