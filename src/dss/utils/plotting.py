...
def plot_network(
    ...
    *,
    ...
    def rel_luminance(rgba: tuple[float, float, float, float]) -> float:
        r, g, b, _ = rgba
    ...
    # Build a mapper that converts node "colours" values to RGBA via cmap
    norm = colors.Normalize(vmin=vmin, vmax=vmax, clip=True)
    cmap_obj = cm.get_cmap(cmap)
    rgba_per_node = [cmap_obj(norm(val)) for val in colours]
    ...
    if highlight_nodes:
        ...
        highlight_sizes = [sizes[node_list_all.index(n)] for n in nodelist]
        highlight_colours = [colours[node_list_all.index(n)] for n in nodelist]
        nx.draw_networkx_nodes(
            G,
            pos,
            nodelist=nodelist,
    ...
    if show_labels:
        ...
        for idx, n in enumerate(G.nodes()):
            ...
            lum = rel_luminance(rgba_per_node[idx])

            # Default red, switch to a high-contrast color on dark nodes
            if lum < 0.35:
                label_color = "white"  # or "yellow"
            else:
                label_color = "#b22222"  # firebrick
